        ...
        <div v-for="game in storeLobby.games" :key="game.id" class="flex ps-2 pe-1">
            <div class="flex flex-col grow">
                <div class="text-base pe-4 grow leading-10 flex space-x-2">
                    <span class="pl-1">{{ game.player1Nickname }}</span>
                </div>
                <span class="text-xs ps-1 pb-2 -mt-1 text-gray-500">
                    {{ new Date(game.lobby_timestamp).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit", second:
                    "2-digit" }) }}</span>
            </div>
            <div class="py-1 flex items-center min-w-[1.9rem]">
                ...
                <button v-show="storeLobby.canJoinGame(game)" 
                  type="button"
                  class="rounded bg-cyan-500 p-2 m-0.5 text-white flex items-center space-x-2" 
